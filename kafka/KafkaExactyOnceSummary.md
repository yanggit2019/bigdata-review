...
消息队列确保写和读的准确是非常重要的, 但 kafka 的多分区的特性使之变得困难.

对于写入消息来说, kafka 可以通过设定 ack 为 0 来保证 at most once, 即至多一次写入, 也就是不重复, 但这样很可能因为写入失败而丢失数据. 当然也可以通过设定 ack 为 -1 来保证 at leat once, 即至少一次写入, 也就是不丢失数据, 但这样可能因为副本同步成功后 leader 分区挂掉而无法向 producer 返回 ack 而导致 producer 重复生产数据. kafka 在 0.11 版本引入了幂等性和事务, 幂等性和 `ack = -1` 可以保证单会话单分区的写入的精准一次性, 再加上 kafka 写入事务可以保证跨会话跨分区的精准一次性.

## 哪些因素影响幂等性

在写入一端: 使用 kafka 时,需要保证 exactly-once 语义. 要知道在分布式系统中, 出现网络分区是不可避免的, 如果 kafka broker 在回复 ack 时, 出现网络故障或者是 full gc 导致 ack timeout, producer 将会重发, 如何保证 producer 重试时不造成重复或乱序? 又或者 producer 挂了, 新的 producer 并没有旧 producer 的状态数据, 这个时候如何保证 exactly-once? 

在读取一端: 即使 kafka 写入的消息满足了幂等, consumer 拉取到消息后, 把消息交给线程池 workers, workers 线程对 message 的处理可能包含异步操作, 又会出现以下情况: 
...
前面介绍 kafka 分区和副本的时候, 有提到过一个 producer 客户端有一个 ack 的配置: 
...
要达到最严格的无消息丢失配置, 需要配置 `ack = -1` , 这时会等待 ISR 所有副本同步完成再返回 ack. 

但此时还有可能 ISR 中只有 leader 一个, 写入 leader 后就返回了 ack, 这时出现 leader 宕机, 其他副本也没有同步, 最终导致数据丢失, 变成了和 `ack = 1` 一样的情况.
...
我们之前提到过, producer 发送到 kafka broker 的时候, 是有多种可能会失败的, 而回调函数能准确告诉你是否确认发送成功, 当然这依托于 ack 和 `min.insync.replicas` 的配置. 而当数据发送丢失的时候, 就可以进行手动重发或其他操作, 从而确保生产者发送成功. 
